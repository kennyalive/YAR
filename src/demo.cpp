...
void Vk_Demo::update_uniform_buffer() {
    ...
    // Update mvp matrix.
    Matrix3x4 model = rotate_y(Matrix3x4::identity, (float)time * radians(30.f));
    Matrix3x4 view = look_at_transform(Vector(0, 0.f, 3.0), Vector(0), Vector(0, 1, 0));
    float aspect_ratio = (float)vk.surface_size.width / (float)vk.surface_size.height;
    Matrix4x4 perspective = perspective_transform_opengl_z01(radians(45.f), aspect_ratio, 0.1f, 50.0f);
