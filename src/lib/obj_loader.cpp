    ...
    const Triangle_Mesh_Load_Params params, std::vector<Obj_Mesh>& obj_meshes)
{
    ...
    obj_meshes.back().name = shape.name.c_str();
    ...
    const std::vector<String>* ignore_geometry_names)
{
    ...
    const String mtl_dir = to_string(fs::path(obj_file_path).parent_path());

    if (!tinyobj::LoadObj(&attrib, &shapes, &materials, &warn, &err, obj_file_path, mtl_dir.data()))
        error("failed to load obj model: %s", obj_file_path);
    ...
    for (auto [i, tinyobj_mtl] : enumerate(materials)) {
        ...
        if (!tinyobj_mtl.diffuse_texname.empty())
            mtl.diffuse_texture = tinyobj_mtl.diffuse_texname.c_str();
    }
