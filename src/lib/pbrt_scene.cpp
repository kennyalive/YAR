...
static Material_Handle import_pbrt_material(const pbrt::Material::SP pbrt_material, Scene* scene)
{
    ...
    if (auto translucent = std::dynamic_pointer_cast<pbrt::TranslucentMaterial>(pbrt_material)) {
        ...
        if (!reflect.is_black()) {
            mtl.diffuse_reflectance = init_rgb_parameter_from_texture_or_constant(scene, translucent->map_kd, translucent->kd);
            mtl.diffuse_reflectance.scale = reflect;
            set_constant_parameter(mtl.specular_reflectance, ColorRGB(&translucent->ks.x));
            mtl.specular_reflectance.scale = reflect;
        }
        if (!transmit.is_black()) {
            mtl.diffuse_transmission = init_rgb_parameter_from_texture_or_constant(scene, translucent->map_kd, translucent->kd);
            mtl.diffuse_transmission.scale = transmit;
            set_constant_parameter(mtl.specular_transmission, ColorRGB(&translucent->ks.x));
            mtl.specular_transmission.scale = transmit;
        }
