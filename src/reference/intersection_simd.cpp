...
void Triangle_Intersection_4x::min(const Triangle_Intersection_4x& other)
{
    __m128 cmp_mask = _mm_cmp_ps(other.t, t, _CMP_LT_OQ);
    t = _mm_blendv_ps(t, other.t, cmp_mask);
    bx = _mm_blendv_ps(bx, other.bx, cmp_mask);
    by = _mm_blendv_ps(by, other.by, cmp_mask);
    bz = _mm_blendv_ps(bz, other.bz, cmp_mask);
    triangle_index = _mm_blendv_epi8(triangle_index, other.triangle_index, _mm_castps_si128(cmp_mask));
}

void Triangle_Intersection_4x::reduce(float* distance, Vector3* barycentrics, uint32_t* primitive_index) const
{
    // reduce from 4 to 2 elements
    __m128 t_2, bx_2, by_2, bz_2;
    __m128i i_2;
    {
        __m128 t0_2 = t;
        __m128 t1_2 = _mm_shuffle_ps(t, t, 0xee);
        __m128 cmp_mask_2 = _mm_cmp_ps(t1_2, t0_2, _CMP_LT_OQ);
        t_2 = _mm_blendv_ps(t0_2, t1_2, cmp_mask_2);

        __m128 bx0_2 = bx;
        __m128 bx1_2 = _mm_shuffle_ps(bx, bx, 0xee);
        bx_2 = _mm_blendv_ps(bx0_2, bx1_2, cmp_mask_2);

        __m128 by0_2 = by;
        __m128 by1_2 = _mm_shuffle_ps(by, by, 0xee);
        by_2 = _mm_blendv_ps(by0_2, by1_2, cmp_mask_2);

        __m128 bz0_2 = bz;
        __m128 bz1_2 = _mm_shuffle_ps(bz, bz, 0xee);
        bz_2 = _mm_blendv_ps(bz0_2, bz1_2, cmp_mask_2);

        __m128i i0_2 = triangle_index;
        __m128i i1_2 = _mm_shuffle_epi32(triangle_index, 0xee);
        i_2 = _mm_blendv_epi8(i0_2, i1_2, _mm_castps_si128(cmp_mask_2));
    }
...
Triangle_Intersection_4x intersect_triangle_watertight_4x(const Ray& ray, const __m128 px[3], const __m128 py[3], const __m128 pz[3])
{
    ...
    __m128 p2t_z, x2, y2;
    {
        __m128 origin_x = _mm_broadcast_ss(&ray.origin.x);
        __m128 origin_y = _mm_broadcast_ss(&ray.origin.y);
        __m128 origin_z = _mm_broadcast_ss(&ray.origin.z);

        float perm_masks[6] = { 0, 0,  -1, 0,  0, -1 };
        __m128 x_perm_mask0 = _mm_set1_ps(perm_masks[2 * kx + 0]);
        __m128 x_perm_mask1 = _mm_set1_ps(perm_masks[2 * kx + 1]);
        __m128 y_perm_mask0 = _mm_set1_ps(perm_masks[2 * ky + 0]);
        __m128 y_perm_mask1 = _mm_set1_ps(perm_masks[2 * ky + 1]);
        __m128 z_perm_mask0 = _mm_set1_ps(perm_masks[2 * kz + 0]);
        __m128 z_perm_mask1 = _mm_set1_ps(perm_masks[2 * kz + 1]);

        __m128 sx4 = _mm_broadcast_ss(&sx);
        __m128 sy4 = _mm_broadcast_ss(&sy);

        {
            __m128 p0t_xx = _mm_sub_ps(px[0], origin_x);
            __m128 p0t_yy = _mm_sub_ps(py[0], origin_y);
            __m128 p0t_zz = _mm_sub_ps(pz[0], origin_z);
            __m128 p0t_x = _mm_blendv_ps(_mm_blendv_ps(p0t_xx, p0t_yy, x_perm_mask0), p0t_zz, x_perm_mask1);
            __m128 p0t_y = _mm_blendv_ps(_mm_blendv_ps(p0t_xx, p0t_yy, y_perm_mask0), p0t_zz, y_perm_mask1);
            p0t_z        = _mm_blendv_ps(_mm_blendv_ps(p0t_xx, p0t_yy, z_perm_mask0), p0t_zz, z_perm_mask1);
            x0 = _mm_fmadd_ps(sx4, p0t_z, p0t_x);
            y0 = _mm_fmadd_ps(sy4, p0t_z, p0t_y);
        }
        {
            __m128 p1t_xx = _mm_sub_ps(px[1], origin_x);
            __m128 p1t_yy = _mm_sub_ps(py[1], origin_y);
            __m128 p1t_zz = _mm_sub_ps(pz[1], origin_z);
            __m128 p1t_x = _mm_blendv_ps(_mm_blendv_ps(p1t_xx, p1t_yy, x_perm_mask0), p1t_zz, x_perm_mask1);
            __m128 p1t_y = _mm_blendv_ps(_mm_blendv_ps(p1t_xx, p1t_yy, y_perm_mask0), p1t_zz, y_perm_mask1);
            p1t_z        = _mm_blendv_ps(_mm_blendv_ps(p1t_xx, p1t_yy, z_perm_mask0), p1t_zz, z_perm_mask1);
            x1 = _mm_fmadd_ps(sx4, p1t_z, p1t_x);
            y1 = _mm_fmadd_ps(sy4, p1t_z, p1t_y);
        }
        {
            __m128 p2t_xx = _mm_sub_ps(px[2], origin_x);
            __m128 p2t_yy = _mm_sub_ps(py[2], origin_y);
            __m128 p2t_zz = _mm_sub_ps(pz[2], origin_z);
            __m128 p2t_x = _mm_blendv_ps(_mm_blendv_ps(p2t_xx, p2t_yy, x_perm_mask0), p2t_zz, x_perm_mask1);
            __m128 p2t_y = _mm_blendv_ps(_mm_blendv_ps(p2t_xx, p2t_yy, y_perm_mask0), p2t_zz, y_perm_mask1);
            p2t_z        = _mm_blendv_ps(_mm_blendv_ps(p2t_xx, p2t_yy, z_perm_mask0), p2t_zz, z_perm_mask1);
            x2 = _mm_fmadd_ps(sx4, p2t_z, p2t_x);
            y2 = _mm_fmadd_ps(sy4, p2t_z, p2t_y);
        }
    }

    __m128 e0 = _mm_mul_ps(y1, x2);
    __m128 e0_err = _mm_fmsub_ps(y1, x2, e0);
    e0 = _mm_fmsub_ps(x1, y2, e0);
    e0 = _mm_sub_ps(e0, e0_err);

    __m128 e1 = _mm_mul_ps(y2, x0);
    __m128 e1_err = _mm_fmsub_ps(y2, x0, e1);
    e1 = _mm_fmsub_ps(x2, y0, e1);
    e1 = _mm_sub_ps(e1, e1_err);

    __m128 e2 = _mm_mul_ps(y0, x1);
    __m128 e2_err = _mm_fmsub_ps(y0, x1, e2);
    e2 = _mm_fmsub_ps(x0, y1, e2);
    e2 = _mm_sub_ps(e2, e2_err);

    // If any of e0/e1/e2 is zero then re-do computation in double precision.
    int is_zero_edge;
    {
        __m128i mask = _mm_set1_epi32(0x7fffffff); // all bits except sign bit, so we detect both +0 and -0
        is_zero_edge  = _mm_testz_si128(_mm_castps_si128(e0), mask);
        is_zero_edge |= _mm_testz_si128(_mm_castps_si128(e1), mask);
        is_zero_edge |= _mm_testz_si128(_mm_castps_si128(e2), mask);
    }
    if (is_zero_edge) {
        __m256d x0_d = _mm256_cvtps_pd(x0);
        __m256d x1_d = _mm256_cvtps_pd(x1);
        __m256d x2_d = _mm256_cvtps_pd(x2);
        __m256d y0_d = _mm256_cvtps_pd(y0);
        __m256d y1_d = _mm256_cvtps_pd(y1);
        __m256d y2_d = _mm256_cvtps_pd(y2);

        __m256d e0_d = _mm256_mul_pd(y1_d, x2_d);
        e0_d = _mm256_fmsub_pd(x1_d, y2_d, e0_d);
        e0 = _mm256_cvtpd_ps(e0_d);

        __m256d e1_d = _mm256_mul_pd(y2_d, x0_d);
        e1_d = _mm256_fmsub_pd(x2_d, y0_d, e1_d);
        e1 = _mm256_cvtpd_ps(e1_d);

        __m256d e2_d = _mm256_mul_pd(y0_d, x1_d);
        e2_d = _mm256_fmsub_pd(x0_d, y1_d, e2_d);
        e2 = _mm256_cvtpd_ps(e2_d);
    }

    __m128 zero = _mm_setzero_ps();

    // Check if edge values have the same side. If it's not the case then there is no intersection.
    __m128 disabled_lanes_mask;
    {
        __m128 lt_zero = _mm_cmp_ps(e0, zero, _CMP_LT_OQ);
        lt_zero = _mm_or_ps(lt_zero, _mm_cmp_ps(e1, zero, _CMP_LT_OQ));
        lt_zero = _mm_or_ps(lt_zero, _mm_cmp_ps(e2, zero, _CMP_LT_OQ));

        __m128 gt_zero = _mm_cmp_ps(e0, zero, _CMP_GT_OQ);
        gt_zero = _mm_or_ps(gt_zero, _mm_cmp_ps(e1, zero, _CMP_GT_OQ));
        gt_zero = _mm_or_ps(gt_zero, _mm_cmp_ps(e2, zero, _CMP_GT_OQ));

        disabled_lanes_mask = _mm_and_ps(lt_zero, gt_zero);
        if (_mm_movemask_ps(disabled_lanes_mask) == 0xf)
            return Triangle_Intersection_4x::no_intersection();
    }

    // Compute: det = e0 + e1 + e2
    __m128 det;
    {
        det = _mm_add_ps(e0, e1);
        det = _mm_add_ps(det, e2);

        __m128 det_is_zero = _mm_cmp_ps(det, zero, _CMP_EQ_OQ);
        disabled_lanes_mask = _mm_or_ps(disabled_lanes_mask, det_is_zero);
        if (_mm_movemask_ps(disabled_lanes_mask) == 0xf)
            return Triangle_Intersection_4x::no_intersection();
    }
    ...
    __m128 t_scaled;
    {
        __m128 sz4 = _mm_broadcast_ss(&sz);
        __m128 z0 = _mm_mul_ps(sz4, p0t_z);
        __m128 z1 = _mm_mul_ps(sz4, p1t_z);
        __m128 z2 = _mm_mul_ps(sz4, p2t_z);

        t_scaled = _mm_mul_ps(e0, z0);
        t_scaled = _mm_fmadd_ps(e1, z1, t_scaled);
        t_scaled = _mm_fmadd_ps(e2, z2, t_scaled);
    }

    // Check if there is no intersection based on signs of det and t_scaled.
    {
        __m128i sign_bits_mask = _mm_set1_epi32(0x80000000);

        __m128i xored_sign_bits = _mm_castps_si128(_mm_xor_ps(det, t_scaled));
        xored_sign_bits = _mm_and_epi32(xored_sign_bits, sign_bits_mask);

        __m128i opposite_signs_mask = _mm_cmpeq_epi32(xored_sign_bits, sign_bits_mask);
        disabled_lanes_mask = _mm_or_ps(disabled_lanes_mask, _mm_castsi128_ps(opposite_signs_mask));
        if (_mm_movemask_ps(disabled_lanes_mask) == 0xf)
            return Triangle_Intersection_4x::no_intersection();
    }

    __m128 all_infinity = _mm_set1_ps(Infinity);
    __m128 all_ones = _mm_set1_ps(1.f);

    __m128 inv_det = _mm_div_ps(all_ones, det);
    __m128 t = _mm_mul_ps(inv_det, t_scaled);

    Triangle_Intersection_4x intersection;
    intersection.t = _mm_blendv_ps(t, all_infinity, disabled_lanes_mask);
    intersection.bx = _mm_mul_ps(e0, inv_det);
    intersection.by = _mm_mul_ps(e1, inv_det);
    intersection.bz = _mm_mul_ps(e2, inv_det);
    return intersection;
}
