    ...
    const std::unordered_map<uint32_t, uint32_t>& node_index_to_primitive_count)
{
    ...
    String path = current_path.empty() ? string_printf("%u", primitive_count) : string_printf("%s %u", current_path.data(), primitive_count);
    KdNode node = kdtree.nodes[node_index];
    if (node.is_leaf()) {
        if (node.get_primitive_count() > 0) {
            int depth = (int)std::count(path.data(), path.data() + path.size(), ' ');
    ...
    bool below_child_empty = kdtree.nodes[below_child].is_leaf() && kdtree.nodes[below_child].get_primitive_count() == 0;
    ...
    print_primitive_subdivisions_for_subtree(kdtree, above_child_empty ? string_printf("%s*", path.data()) : path, below_child, node_index_to_primitive_count);
    print_primitive_subdivisions_for_subtree(kdtree, below_child_empty ? string_printf("%s*", path.data()) : path, above_child, node_index_to_primitive_count);
}
