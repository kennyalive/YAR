...
#ifdef VK_MINILIB_HEADER
#include VK_MINILIB_HEADER
#else
#include "minilib.h"
#endif
...
#define VK_CHECK_RESULT(result) if (result < 0 && vk.error) vk.error(string_concat("Error: ", vk_result_to_string(result)));
#define VK_CHECK(function_call) { VkResult result = function_call;  VK_CHECK_RESULT(result); }

using Vk_Error_Func = void (*)(const String& error_message);
...
struct Vk_Init_Params {
    ...
    Span<const char* const> device_extensions;
    const VkBaseInStructure* device_create_info_pnext = nullptr;
    Span<const VkFormat> supported_surface_formats;
...
VkPipeline vk_create_compute_pipeline(
    ...
    Span<const VkDescriptorSetAndBindingMappingEXT> binding_mappings,
    const char* name
);
...
struct Vk_Shader_Module {
    Vk_Shader_Module(const String& spirv_file);
    ~Vk_Shader_Module();
